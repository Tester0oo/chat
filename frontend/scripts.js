...
function renderMessages(msgs)
{
    ...
    msgs.forEach(
        m=> {
            ...
            if (m.sender.id==sessionStorage.getItem("currentUser"))
                {style_msg="my-message"}
            messageDiv.classList.add('message',style_msg)
            messageDiv.innerHTML=`<div class="sender">${m.sender.name}</div><div>${m.text}</div><div>${m.timestamp} </div>`
...
async function sendMessage(e)
{
  e.preventDefault()
  const currentRoom = sessionStorage.getItem("currentRoom") || "general";
  const msg = {sender_id: sessionStorage.getItem("currentUser"),text: messageInput.value, room: currentRoom}
  const options = {
    method: "POST",
...
async function loginUser(e)
{
 ...
 {
      await fetch(`http://localhost:8000/messages/poll?since=${Date.now()-3000}`)
 }
